      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          {feedback.map((item, idx) => (
            <Card key={idx} className="group border-none shadow-2xl rounded-[3rem] bg-white p-12 relative overflow-hidden transition-all duration-500 hover:-translate-y-3">
              ...
              <CardContent className="space-y-12 p-0 relative z-10">
                ...
                <p className="text-3xl font-medium leading-relaxed italic text-foreground/90 font-headline">
                  "{item.quote}"
                </p>
                <div className="pt-10 border-t border-muted flex items-center justify-between">
                  <div>
                    <h4 className="text-2xl font-bold text-primary font-headline">{item.author}</h4>
                    <p className="text-muted-foreground font-bold text-base uppercase tracking-widest">{item.role}</p>
                  </div>
