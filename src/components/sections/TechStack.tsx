...
const stack = [
  {
    ...
    hover: "hover:rotate-3 hover:scale-110"
  },
  {
    category: "Backend",
    icon: Server,
  ...
  },
  {
    ...
    hover: "hover:skew-x-3 hover:scale-110"
  },
  {
...
export function TechStack() {
  return (
    <section id="tech-stack" className="py-32 bg-background relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="text-center max-w-4xl mx-auto mb-24 space-y-8 animate-in fade-in slide-in-from-top duration-1000">
          ...
          <p className="text-2xl text-muted-foreground leading-relaxed font-semibold max-w-2xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
          {stack.map((group, idx) => (
            <div 
              key={idx} 
              className={`group p-12 ${group.bg} text-white rounded-[4rem] border-none shadow-2xl transition-all duration-500 ${group.hover} relative overflow-hidden`}
            >
              ...
              <div className="flex items-center gap-6 mb-12">
                <div className={`w-20 h-20 rounded-[1.8rem] bg-white/20 backdrop-blur-md flex items-center justify-center transition-transform group-hover:rotate-[360deg] duration-700 shadow-xl`}>
                  <group.icon className={`w-10 h-10 ${group.color}`} />
                </div>
                <h3 className="text-3xl font-black font-headline italic">{group.category}</h3>
              </div>
              ...
              <div className="flex flex-wrap gap-4 relative z-10">
                {group.items.map((tech, i) => (
                  <Badge 
                    key={i} 
                    variant="outline" 
                    className="px-6 py-3 text-lg font-black rounded-2xl border-2 border-white/30 text-white transition-all duration-300 hover:bg-white hover:text-foreground cursor-default shadow-lg"
                  >
                    {tech}
                  </Badge>
                ))}
